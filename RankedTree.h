...
#ifndef RANKEDTREETEST_RANKEDTREE_H
#define RANKEDTREETEST_RANKEDTREE_H
...
void RankedTree::insert(int score) {
    if (!root) {
...
int RankedTree::getHeight(AVLRankedNode *node) const {
    int height = 0;
    if (node) {
...
void RankedTree::deleteSubTree(AVLRankedNode *node) {
    if (node) {
...
int RankedTree::balanceFactor(AVLRankedNode *root) const {
    int balance = 0;
    if (root) {
...
void RankedTree::printInOrder() const {
    return this->printInOrder(this->root);
}


void RankedTree::printInOrder(AVLRankedNode *root) const {
    if (root) {
...
void RankedTree::update(AVLRankedNode *node) {
    while (node) {
...
void RankedTree::roll_LL(AVLRankedNode *node1) {
...
void RankedTree::roll_RR(AVLRankedNode *node) {
...
void RankedTree::roll_LR(AVLRankedNode *node1) {
    AVLRankedNode *node = node1->getLeft();
    roll_RR(node);
    roll_LL(node1);
}

void RankedTree::roll_RL(AVLRankedNode *node1) {
    AVLRankedNode *node = node1->getRight();
    roll_LL(node);
    roll_RR(node1);
}

void RankedTree::roll(AVLRankedNode *node) {
...
int RankedTree::getScore(int k) {
    if (!this->root)
        return 0;
    return this->getScore(root, k);
}

int RankedTree::getScore(AVLRankedNode *node, int k) {
    if (k <= 0 || !node)
        return 0;
    if (node->getRank() <= k) {
...
#endif //RANKEDTREETEST_RANKEDTREE_H
